...
export default function Home() {
  ...
  const { genreCounts, artistCounts, yearCounts, albumCounts } = useMemo(() => {
    ...
    const a: Record<string, { count: number; image?: string }> = {};
    ...
    for (const t of tracks) {
      ...
      for (const art of t.artists) {
        if (!a[art]) a[art] = { count: 0, image: undefined };
  
        if (!a[art].image && t.artistImages?.[art]) {
          a[art].image = t.artistImages[art]!;
        }
  ...
  const flattenedArtistCounts = useMemo(() => {
    ...
    for (const [artist, data] of Object.entries(artistCounts)) {
      result[artist] = data.count;
    }
    return result;
  }, [artistCounts]);
